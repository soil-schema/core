...
export const entityOpen = (context: Context): string => {
  return `struct ${context.get('name')}: Codable {`;
}
...
export default (generator: Generator) => {
  generator
    .template(new Template('swift', 'entity', entityTemplate))
    .renderer('entity', 'file-header', entityFileHeader)
    .renderer('entity', 'open', entityOpen)
    .renderer('entity', 'init', entityInit)
    .renderer('entity', 'close', () => '}')
